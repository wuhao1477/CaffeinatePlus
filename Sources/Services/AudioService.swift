...
class AudioService: ObservableObject {
  ...
  private let blackHoleDriverName = "BlackHole"
  private let aggregateDeviceName = "CaffeinatePlus Audio"
  private let deviceSwitchDelayMicroseconds: UInt32 = 500_000  // 500ms
  ...
  func stopRouting() {
    ...
    // 2. 等待设备切换完成
    usleep(deviceSwitchDelayMicroseconds)
  ...
  private func findBlackHoleDevice() throws -> AudioDeviceID {
    ...
    for deviceID in devices {
      if let name = getDeviceName(deviceID),
        name.contains(blackHoleDriverName)
      {
        return deviceID
      }
    }
  ...
  private func createAggregateDevice(
    ...
    guard let outputUID = getDeviceUID(outputDeviceID),
      let virtualUID = getDeviceUID(virtualDeviceID)
    else {
